import { BaseQuery, BaseFilter, UserError } from '@cubejs-backend/schema-compiler';
...
class ClickhouseDSFilter extends BaseFilter {
  ...
  filterToWhere() {
    ...
    let sql = this.cube().sql?.toString();
    if (sql) {
...
export class ClickhouseDSQuery extends BaseQuery {
  newFilter(filter) {
